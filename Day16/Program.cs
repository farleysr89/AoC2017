...
namespace Day16
{
    internal class Program
    {
        ...
        private static void SolvePart2()
        {
            ...
            var data = input.Split(',').ToList();
            var programs = new StringBuilder("abcdefghijklmnop");
            var appeared = new HashSet<string> { programs.ToString() };
            var loop = 0;
            for (var i = 0; i < 1000000000; i++)
            {
                ...
                if (appeared.Contains(programs.ToString()))
                {
                    loop = i + 1;
                    break;
                }
                else appeared.Add(programs.ToString());
            }
            for (var i = 0; i < 1000000000 % loop; i++)
            {
                foreach (var s in data)
                {
                    switch (s[0])
